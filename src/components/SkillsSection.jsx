        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((category, key) => (
            <button
              ...
              className={cn(
                "px-5 py-2 rounded-full hover:scale-105 cursor-pointer transition-all duration-300 capitalize",
                activeCategory === category
                  ? "bg-primary text-primary-foreground"
                  : "bg-primary/60 text-forefround hover:bg-[#5b5ba3]"
              )}
            >
              {category}
            </button>
          ...
          {filteredSkills.map((skill, key) => (
            ...
            >
              <div className="text-left mb-4">
                <h3 className="font-semibold text-lg"><span className="text-glow text-foreground">&lt;</span><span className="text-glow text-foreground">&gt;</span> {skill.name}</h3>
              </div>
